 ...
 *
 * @version $Id: XMLNSDocumentScannerImpl.java 495747 2007-01-12 21:48:00Z
 *          mrglavas $
 */
public class XMLNSDocumentScannerImplMod extends XMLDocumentScannerImplMod {

  /**
   * If is true, the dtd validator is no longer in the pipeline and the scanner
   * should bind namespaces
   */
  ...
  /**
   * The scanner is responsible for removing DTD validator from the pipeline if it
   * is not needed.
   *
  ...
  protected void scanAttribute(XMLAttributesImpl attributes) throws IOException, XNIException {
    ...
    // value are the same
    boolean isSameNormalizedAttr = scanAttributeValue(this.fTempString, fTempString2, fAttributeQName.rawname,
  ...
  protected final class NSContentDispatcher extends ContentDispatcher {
     ...
     * that handles scanning for the root element. This method will also attempt to
     * remove DTD validator from the pipeline, if there is no DTD grammar. If DTD
     * validator is no longer in the pipeline bind namespaces in the scanner.
     *
     *
    ...
    /**
     * Re-configures pipeline by removing the DTD validator if no DTD grammar
     * exists. If no validator exists in the pipeline or there is no DTD grammar,
     * namespace binding is performed by the scanner in the enclosing class.
     */
